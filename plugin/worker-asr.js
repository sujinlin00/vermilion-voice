...
// lib/fbank.js
var PREEMPH_COEFF = 0.97;
function fft(re, im2) {
  const n = re.length;
  for (let i = 0, j2 = 0; i < n; i++) {
    if (j2 > i) {
      [re[i], re[j2]] = [re[j2], re[i]];
      [im2[i], im2[j2]] = [im2[j2], im2[i]];
    }
    let m = n >> 1;
    while (m >= 1 && j2 >= m) {
      j2 -= m;
  ...
  for (let len = 2; len <= n; len <<= 1) {
    ...
    for (let i = 0; i < n; i += len) {
      for (let j2 = 0; j2 < half; j2++) {
        const idx = i + j2;
        const idxH = idx + half;
        const cos = Math.cos(angle * j2);
        const sin = Math.sin(angle * j2);
        const tr = re[idxH] * cos - im2[idxH] * sin;
        const ti = re[idxH] * sin + im2[idxH] * cos;
        re[idxH] = re[idx] - tr;
        im2[idxH] = im2[idx] - ti;
        re[idx] += tr;
        im2[idx] += ti;
...
function makeMelFilterbank(sampleRate, nfft, nMels, lowFreq = 20) {
  ...
  for (let m = 0; m < nMels; m++) {
    ...
    for (let k2 = leftBin; k2 < rightBin && k2 < numBins; k2++) {
      let w2 = 0;
      if (k2 < centerF && ascDen > 0) {
        w2 = (k2 - leftF) / ascDen;
      } else if (descDen > 0) {
        w2 = (rightF - k2) / descDen;
      }
      if (w2 > 0) row[k2] = Math.min(w2, 1);
    }
    rows[m] = row;
  }
  return { rows, numBins };
}
function loadCMVN(text) {
...
var FbankProcessor = class {
   ...
   */
  constructor(opts = {}) {
    this.sampleRate = opts.fs || 16e3;
    this.nMels = opts.n_mels || 80;
    this.frameLengthMs = opts.frame_length_ms || 25;
    this.frameShiftMs = opts.frame_shift_ms || 10;
    this.dither = opts.dither != null ? opts.dither : 1;
    this.lfrM = opts.lfr_m || 7;
    this.lfrN = opts.lfr_n || 6;
    this.cmvn = opts.cmvn || null;
    this.frameLength = Math.round(this.sampleRate * this.frameLengthMs / 1e3);
    this.frameShift = Math.round(this.sampleRate * this.frameShiftMs / 1e3);
    this.fftSize = 1;
    while (this.fftSize < this.frameLength) this.fftSize <<= 1;
    ...
    for (let f = 0; f < numFrames; f++) {
      ...
      if (ditherStd > 0) {
        for (let i = 0; i < frameLen; i++) {
          ...
          u2 = Math.random();
          re[i] += Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2) * ditherStd;
        }
        frameSum = 0;
      ...
      for (let m = 0; m < this.nMels; m++) {
        let sum = 0;
        const row = melRows[m];
        for (let k2 = 0; k2 < numBins; k2++) {
          sum += row[k2] * (re[k2] * re[k2] + im2[k2] * im2[k2]);
        }
  ...
  applyLFR(feat) {
    ...
    const D2 = this.nMels;
    const T = feat.length / D2;
    if (!Number.isInteger(T)) throw new Error(`feat length ${feat.length} not divisible by dim ${D2}`);
    const leftPad = lfrM - 1 >> 1;
    ...
    const getFrame = (idx) => {
      ...
      return feat.subarray(clamped * D2, (clamped + 1) * D2);
    };
    const outDim = lfrM * D2;
    const out = new Float32Array(T_lfr * outDim);
    for (let i = 0; i < T_lfr; i++) {
      const outOff = i * outDim;
      for (let j2 = 0; j2 < lfrM; j2++) {
        const frameIdx = i * lfrN + j2;
        if (frameIdx < effectiveT) {
          out.set(getFrame(frameIdx), outOff + j2 * D2);
        } else {
          out.set(getFrame(T - 1 + leftPad), outOff + j2 * D2);
        }
      }
    }
...
// lib/streaming_fbank.js
var StreamingFbankProcessor = class extends FbankProcessor {
  constructor(opts = {}) {
    super(opts);
  ...
  accept_waveform(waveform, is_final = false) {
    ...
    const D2 = this.nMels;
    if (T === 0) {
      return { feat: new Float32Array(0), featLen: 0, dim: D2 };
    }
    if (lfrM !== 1 || lfrN !== 1) {
      if (this.lfr_splice_cache.length === 0) {
        const leftPad = lfrM - 1 >> 1;
        const frame0 = feats.slice(0, D2);
      ...
      if (cacheLen + T < lfrM) {
        for (let i = 0; i < T; i++) {
          this.lfr_splice_cache.push(feats.slice(i * D2, (i + 1) * D2));
        }
        return { feat: new Float32Array(0), featLen: 0, dim: lfrM * D2 };
      }
      const catT = cacheLen + T;
      const catFeats = new Float32Array(catT * D2);
      for (let i = 0; i < cacheLen; i++) {
        catFeats.set(this.lfr_splice_cache[i], i * D2);
      }
      catFeats.set(feats, cacheLen * D2);
      const lfrResult = this._apply_lfr_online(catFeats, catT, D2, is_final);
    ...
    const outDim = lfrM !== 1 || lfrN !== 1 ? lfrM * D2 : D2;
    if (this.cmvn) {
   ...
   */
  _apply_lfr_online(inputs, T, D2, is_final) {
    const lfrM = this.lfrM;
    const lfrN = this.lfrN;
    ...
    for (let i = 0; i < T_lfr; i++) {
      ...
      if (lfrM <= remaining) {
        ...
        for (let j2 = 0; j2 < lfrM; j2++) {
          frame.set(
            inputs.subarray((startFrame + j2) * D2, (startFrame + j2 + 1) * D2),
            j2 * D2
          );
        }
        ...
        if (is_final) {
          const frame = new Float32Array(outDim);
          for (let j2 = 0; j2 < lfrM; j2++) {
            const srcIdx = Math.min(startFrame + j2, T - 1);
            frame.set(
              inputs.subarray(srcIdx * D2, (srcIdx + 1) * D2),
              j2 * D2
            );
          }
          lfrFrames.push(frame);
    ...
    for (let i = spliceStart; i < T; i++) {
      spliceCache.push(inputs.slice(i * D2, (i + 1) * D2));
    }
  ...
  _flushFinalLFR(waveform, is_final) {
    ...
    const cacheLen = this.lfr_splice_cache.length;
    const catFeats = new Float32Array(cacheLen * D2);
    for (let i = 0; i < cacheLen; i++) {
      catFeats.set(this.lfr_splice_cache[i], i * D2);
    }
    const lfrResult = this._apply_lfr_online(catFeats, cacheLen, D2, true);
    this.lfr_splice_cache = lfrResult.splice_cache;
...
var Co = ((i) => typeof __require < "u" ? __require : typeof Proxy < "u" ? new Proxy(i, { get: (e, o) => (typeof __require < "u" ? __require : e)[o] }) : i)(function(i) {
...
var Or = (i, e) => {
  for (var o in e) an(i, o, { get: e[o], enumerable: true });
};
var Qa = (i, e, o, t) => {
  if (e && typeof e == "object" || typeof e == "function") for (let r of jd(e)) !Kd.call(i, r) && r !== o && an(i, r, { get: () => e[r], enumerable: !(t = qd(e, r)) || t.enumerable });
  return i;
};
var rr = (i, e, o) => (o = i != null ? Hd(Xd(i)) : {}, Qa(e || !i || !i.__esModule ? an(o, "default", { value: i, enumerable: true }) : o, i));
var sn = (i) => Qa(an({}, "__esModule", { value: true }), i);
var un;
var ke;
var nr;
var Jd;
...
var fn = O(() => {
  "use strict";
  un = /* @__PURE__ */ new Map(), ke = [], nr = (i, e, o) => {
    ...
    let e = i.executionProviders || [], o = e.map((u) => typeof u == "string" ? u : u.name), t = o.length === 0 ? ke : o, r, n = [], s = /* @__PURE__ */ new Set();
    for (let u of t) {
      let l = await Jd(u);
...
var No = O(() => {
  "use strict";
  rs();
  ns = "warning", Gt = { wasm: {}, webgl: {}, webgpu: {}, versions: { common: es }, set logLevel(i) {
    if (i !== void 0) {
  ...
  Object.defineProperty(Gt, "logLevel", { enumerable: true });
});
var z;
var os = O(() => {
  "use strict";
...
var ss = O(() => {
  ...
  is = (i, e) => {
    ...
    if (t != null) {
      ...
      let f = n * r, p = 0, d = f, y = f * 2, T = -1;
      s === "RGBA" ? (p = 0, d = f, y = f * 2, T = f * 3) : s === "RGB" ? (p = 0, d = f, y = f * 2) : s === "RBG" && (p = 0, y = f, d = f * 2);
      for (let v = 0; v < n; v++) for (let S = 0; S < r; S++) {
        let L = (i.data[p++] - l[0]) * u[0], P = (i.data[d++] - l[1]) * u[1], A = (i.data[y++] - l[2]) * u[2], M = T === -1 ? 255 : (i.data[T++] - l[3]) * u[3];
    ...
    if (o != null) {
      ...
      let d = 4, y = 0, T = 1, v = 2, S = 3, L = 0, P = p, A = p * 2, M = -1;
...
var ds = O(() => {
  ...
  Ro = (i, e) => {
    ...
    let a = e.format !== void 0 ? e.format : "RGBA", u = e.tensorFormat !== void 0 && e.tensorFormat !== void 0 ? e.tensorFormat : "RGB", l = o * t, f = u === "RGBA" ? new Float32Array(l * 4) : new Float32Array(l * 3), p = 4, d = 0, y = 1, T = 2, v = 3, S = 0, L = l, P = l * 2, A = -1;
    ...
    return new St({ location: "texture", type: "float32", texture: i, dims: s, download: r, dispose: n });
  }, fs = (i, e) => {
    let { dataType: o, dims: t, download: r, dispose: n } = e;
    return new St({ location: "gpu-buffer", type: o ?? "float32", gpuBuffer: i, dims: t, download: r, dispose: n });
  }, cs = (i, e) => {
    let { dataType: o, dims: t, download: r, dispose: n } = e;
    return new St({ location: "ml-tensor", type: o ?? "float32", mlTensor: i, dims: t, download: r, dispose: n });
  }, ps = (i, e, o) => new St({ location: "cpu-pinned", type: i, data: e, dims: o ?? [e.length] });
});
var Be;
var Sr;
var hs;
...
var bs = O(() => {
  "use strict";
  Be = /* @__PURE__ */ new Map([["float32", Float32Array], ["uint8", Uint8Array], ["int8", Int8Array], ["uint16", Uint16Array], ["int16", Int16Array], ["int32", Int32Array], ["bool", Uint8Array], ["float64", Float64Array], ["uint32", Uint32Array], ["int4", Uint8Array], ["uint4", Uint8Array]]), Sr = /* @__PURE__ */ new Map([[Float32Array, "float32"], [Uint8Array, "uint8"], [Int8Array, "int8"], [Uint16Array, "uint16"], [Int16Array, "int16"], [Int32Array, "int32"], [Float64Array, "float64"], [Uint32Array, "uint32"]]), hs = false, ms = () => {
    if (!hs) {
      hs = true;
...
var xs = O(() => {
  ...
  gs = (i) => {
    ...
    switch (i.location) {
      ...
      case "cpu-pinned":
        return new St({ location: "cpu-pinned", data: i.data, type: i.type, dims: e });
      case "texture":
        return new St({ location: "texture", texture: i.texture, type: i.type, dims: e });
      case "gpu-buffer":
        return new St({ location: "gpu-buffer", gpuBuffer: i.gpuBuffer, type: i.type, dims: e });
      case "ml-tensor":
        return new St({ location: "ml-tensor", mlTensor: i.mlTensor, type: i.type, dims: e });
      default:
    ...
    constructor(e, o, t) {
      ...
      if (typeof e == "object" && "location" in e) switch (this.dataLocation = e.location, r = e.type, n = e.dims, e.location) {
        ...
        if (typeof e == "string") if (r = e, u = t, e === "string") {
          ...
          else if (o instanceof Uint8ClampedArray) if (e === "uint8") a = Uint8Array.from(o);
...
var vs = O(() => {
  ...
  dn = class i {
      ...
      if (typeof o == "object") {
        ...
        if (Array.isArray(o)) {
          ...
          for (let p of this.outputNames) if (f.indexOf(p) !== -1) {
            let d = o[p];
            (d === null || d instanceof yt) && (l = true, s = false, r[p] = d);
          }
          if (l) {
            if (typeof t == "object" && t !== null) n = t;
            else if (typeof t < "u") throw new TypeError("'options' must be an object.");
    ...
    static async create(e, o, t, r) {
      ...
      let [a, u] = await ln(s), l = await a.createInferenceSessionHandler(n, u);
      return Ce(), new i(l);
    }
    startProfiling() {
...
var Ps = O(() => {
  ...
  Zd = "Training backend could not be resolved. Make sure you're using the correct configuration & WebAssembly files.", hn = class i {
    ...
    static async create(e, o) {
      let t = e.evalModel || "", r = e.optimizerModel || "", n = o || {}, [s, a] = await ln(n);
      ...
      if (typeof r == "object") {
        ...
        if (Array.isArray(r)) {
          ...
          for (let p of o) if (f.indexOf(p) !== -1) {
            let d = r[p];
            (d === null || d instanceof yt) && (l = true, u = false, s[p] = d);
          }
          if (l) {
            if (typeof n == "object" && n !== null) a = n;
            else if (typeof n < "u") throw new TypeError("'options' must be an object.");
...
var Mo = {};
Or(Mo, { InferenceSession: () => Yd, TRACE: () => Ts, TRACE_FUNC_BEGIN: () => Fe, TRACE_FUNC_END: () => Ce, Tensor: () => yt, TrainingSession: () => Qd, env: () => z, registerBackend: () => nr });
var Yt = O(() => {
  "use strict";
  ts();
  os();
...
function eh(i) {
  return { verbose: _e.verbose.bind(null, i), info: _e.info.bind(null, i), warning: _e.warning.bind(null, i), error: _e.error.bind(null, i), fatal: _e.fatal.bind(null, i) };
}
function mn(i, e, o, t) {
  let r = Ar[t || ""] || Ar[""];
  Ls[i] < Ls[r.minimalSeverity] || (r.logDateTime && (e = `${(/* @__PURE__ */ new Date()).toISOString()}|${e}`), r.logSourceLocation, th[r.provider].log(i, e, t));
}
var Uo;
var Vo;
var Ls;
...
var Ut = O(() => {
  ...
  }, Ls = { verbose: 1e3, info: 2e3, warning: 4e3, error: 5e3, fatal: 6e3 }, th = { none: new Uo(), console: new Vo() }, $s = { provider: "console", minimalSeverity: "warning", logDateTime: true, logSourceLocation: false }, Ar = { "": $s };
  ((u) => {
    function i(l, f) {
      u("verbose", l, f);
    }
  ...
  tt = _e, gn = class {
    ...
    start() {
      this._started = true, this._timingEvents = [], this._flushTime = bn(), this._flushPointer = 0;
    }
    stop() {
...
function ks(i, e, o) {
  for (let t of o) {
    ...
    if (i.opType === r) {
      for (let l of e) if ((l.domain === n || l.domain === "ai.onnx" && n === "") && rh(l.version, s)) return { opImpl: a, opInit: u };
    }
  }
...
function Ne(i, e) {
  var o, t, r;
  return e ? (i >>>= 0, (r = 0 <= i && i < 256) && (t = Rs[i], t) ? t : (o = J(i, 0, true), r && (Rs[i] = o), o)) : (i |= 0, (r = -128 <= i && i < 128) && (t = Ns[i], t) ? t : (o = J(i, i < 0 ? -1 : 0, false), r && (Ns[i] = o), o));
}
function zt(i, e) {
...
function Ho(i, e, o) {
  ...
  if (typeof e == "number" ? (o = e, e = false) : e = !!e, i === "NaN" || i === "Infinity" || i === "+Infinity" || i === "-Infinity") return e ? ye : Zt;
...
var qo = O(() => {
  ...
  Wo = Ne(-1);
  rt.NEG_ONE = Wo;
  Ws = J(-1, 2147483647, false);
  rt.MAX_VALUE = Ws;
  Hs = J(-1, -1, true);
  rt.MAX_UNSIGNED_VALUE = Hs;
  Ct = J(0, -2147483648, false);
  rt.MIN_VALUE = Ct;
  D = rt.prototype;
  D.toInt = function() {
  ...
  D.toString = function(e) {
    ...
    if (this.isNegative()) if (this.eq(Ct)) {
      var o = zt(e), t = this.div(o), r = t.mul(o).sub(this);
  ...
  D.equals = function(e) {
    return Et(e) || (e = Qt(e)), this.unsigned !== e.unsigned && this.high >>> 31 === 1 && e.high >>> 31 === 1 ? false : this.high === e.high && this.low === e.low;
  };
  D.eq = D.equals;
  ...
  D.multiply = function(e) {
    ...
    var t = this.high >>> 16, r = this.high & 65535, n = this.low >>> 16, s = this.low & 65535, a = e.high >>> 16, u = e.high & 65535, l = e.low >>> 16, f = e.low & 65535, p = 0, d = 0, y = 0, T = 0;
...
var wn = O(() => {
  ...
  w.Builder = function(i) {
    ...
    this.bb = w.ByteBuffer.allocate(e), this.space = e, this.minalign = 1, this.vtable = null, this.vtable_in_use = 0, this.isNested = false, this.object_start = 0, this.vtables = [], this.vector_num_elems = 0, this.force_defaults = false;
  };
  ...
  w.Builder.growByteBuffer = function(i) {
    ...
    var o = e << 1, t = w.ByteBuffer.allocate(o);
  ...
  w.ByteBuffer.prototype.__has_identifier = function(i) {
    ...
    for (var e = 0; e < w.FILE_IDENTIFIER_LENGTH; e++) if (i.charCodeAt(e) != this.readInt8(this.position_ + w.SIZEOF_INT + e)) return false;
    return true;
  };
...
var Pr = O(() => {
  ...
  ((e) => {
    ...
    ((t) => {
      ...
      ((n) => {
        let r;
        ((A) => (A[A.UNDEFINED = 0] = "UNDEFINED", A[A.FLOAT = 1] = "FLOAT", A[A.INT = 2] = "INT", A[A.STRING = 3] = "STRING", A[A.TENSOR = 4] = "TENSOR", A[A.GRAPH = 5] = "GRAPH", A[A.FLOATS = 6] = "FLOATS", A[A.INTS = 7] = "INTS", A[A.STRINGS = 8] = "STRINGS", A[A.TENSORS = 9] = "TENSORS", A[A.GRAPHS = 10] = "GRAPHS", A[A.SPARSE_TENSOR = 11] = "SPARSE_TENSOR", A[A.SPARSE_TENSORS = 12] = "SPARSE_TENSORS"))(r = n.AttributeType || (n.AttributeType = {}));
      })(o = t.fbs || (t.fbs = {}));
...
var Ys = mt((Js) => {
  ...
  var ar = new Array(64), Ks = new Array(123);
  ...
  function mu(i) {
    function e(o, t) {
      ...
      Object.defineProperty(this, "message", { get: function() {
        return o;
      } }), Error.captureStackTrace ? Error.captureStackTrace(this, e) : Object.defineProperty(this, "stack", { value: new Error().stack || "" }), t && hu(this, t);
    }
...
var ei = mt((Jy, xu) => {
  "use strict";
  xu.exports = X;
  var Wt = Se(), Ko, _n = Wt.LongBits, bu = Wt.base64, gu = Wt.utf8;
  function Dr(i, e, o) {
...
var oi = mt((Zy, Au) => {
  "use strict";
  Au.exports = ct;
  var ee = Se(), ni, Ou = ee.LongBits, ph = ee.utf8;
  function re(i, e) {
  ...
  ct._configure = function(i) {
    ...
    ee.merge(ct.prototype, { int64: function() {
      return ri.call(this)[e](false);
    }, uint64: function() {
      return ri.call(this)[e](true);
    }, sint64: function() {
      return ri.call(this).zzDecode()[e](false);
    }, fixed64: function() {
      return _u.call(this)[e](true);
    }, sfixed64: function() {
      return _u.call(this)[e](false);
    } });
  };
});
var Lu = mt((Qy, Du) => {
...
var sr = mt((ix, zu) => {
  "use strict";
  var nt = Vu(), $ = nt.Reader, pt = nt.Writer, b = nt.util, h = nt.roots.default || (nt.roots.default = {});
  h.onnx = function() {
    var i = {};
...
var Y = O(() => {
  ...
  Me = class {
    ...
    static calc(e, o, t, r, n) {
      ...
      if (s) {
        ...
        if (s.length === 0) u.set([], t(e.get([]), o.get([])));
          ...
          let l = new Array(s.length), f = new Array(e.dims.length), p = new Array(o.dims.length), d = 0, y = 0, T = false, v = false;
          e.dims.length === 0 && (d = e.get([]), T = true), o.dims.length === 0 && (y = o.get([]), v = true);
          let S;
          for (let L = 0; L < a; L++) {
            S = L;
    ...
    static longToNumber(e, o) {
      return xe.isLong(e) ? e.toNumber() : e instanceof w.Long ? xe.fromValue({ low: e.low, high: e.high, unsigned: o ?? false }).toNumber() : e;
    }
    static isLong(e) {
...
function Hu(i, e, o) {
  switch (e) {
    ...
    case H.onnx.TensorProto.DataType.INT64:
      return ui(xe.fromBits(i.getUint32(o, true), i.getUint32(o + 4, true), false), e);
    case H.onnx.TensorProto.DataType.DOUBLE:
      return i.getFloat64(o, true);
    case H.onnx.TensorProto.DataType.UINT64:
      return ui(xe.fromBits(i.getUint32(o, true), i.getUint32(o + 4, true), true), e);
    default:
...
var We = O(() => {
  ...
  si = F.experimental.fbs, bt = class i {
    ...
    static fromProto(e) {
      ...
      let o = At.tensorDataTypeFromProto(e.dataType), t = At.tensorDimsFromProto(e.dims), r = new i(t, o);
      if (o === "string") e.stringData.forEach((n, s) => {
        r.data[s] = kr(n);
      });
      else if (e.rawData && typeof e.rawData.byteLength == "number" && e.rawData.byteLength > 0) {
        let n = r.data, s = new DataView(e.rawData.buffer, e.rawData.byteOffset, e.rawData.byteLength), a = Wu(e.dataType), u = e.rawData.byteLength / a;
    ...
    static fromOrtTensor(e) {
      if (!e) throw new Error("cannot construct Value from an empty tensor");
      let o = At.tensorDimsFromORTFormat(e), t = At.tensorDataTypeFromProto(e.dataType()), r = new i(o, t);
      if (t === "string") for (let n = 0; n < e.stringDataLength(); n++) r.data[n] = e.stringData(n);
        ...
        let n = r.data, s = new DataView(e.rawDataArray().buffer, e.rawDataArray().byteOffset, e.rawDataLength()), a = Wu(e.dataType()), u = e.rawDataLength() / a;
...
var ut = O(() => {
  "use strict";
  bh = { version: "", attribute: "attribute", varyingVertex: "varying", varyingFrag: "varying", texture2D: "texture2D", output: "gl_FragColor", outputDeclaration: "" }, gh = { version: "#version 300 es", attribute: "in", varyingVertex: "out", varyingFrag: "in", texture2D: "texture", output: "outputColor", outputDeclaration: "out vec4 outputColor;" };
});
var j = O(() => {
  "use strict";
});
...
var tl = O(() => {
  ...
  pe();
  He();
  Zu = { name: "pack", inputNames: ["A"], inputTypes: [1] }, xh = (i, e) => {
    let o = G(i.session.backend.glContext.version), t = e.dims, r = t.length, n = e.dims.length, s = Bt(n), a = cr("rc", n), u = vh(n, a, t[t.length - 2], t[t.length - 1]), l;
    r === 0 ? l = [1, 1] : r === 1 ? l = [t[0], 1] : l = [t[n - 1], t[n - 2]];
    let f = Th(n, l, a), p = wh(t, a), d = `
        void main() {
      ...
      `;
    return { ...Zu, hasMain: true, output: { dims: e.dims, type: e.type, textureType: 2 }, shaderSource: d };
...
var nl = O(() => {
  ...
  j();
  He();
  Ih = (i) => ({ name: "Reshape (packed)", inputTypes: [2], inputNames: ["A"], cacheHint: `${i}` }), _h = (i, e, o, t) => {
    let r = e.dims, n = t, s = "";
    for (let l = 0; l < 4; l++) {
      let f = "";
      switch (l) {
...
var sl = O(() => {
  ...
  pe();
  He();
  il = { name: "unpack", inputNames: ["A"], inputTypes: [2] }, Ah = (i, e) => {
    let o = e.dims.length, t = cr("rc", o), r = t.slice(-2), n = Bt(o), s = de(), u = e.dims.length === 0 ? "" : Ph(o, t), l = o <= 1 ? "rc" : `vec2(${r.join(",")})`, f = G(i.session.backend.glContext.version), p = `
    ${s}
    void main() {
   ...
   `;
    return { ...il, hasMain: true, output: { dims: e.dims, type: e.type, textureType: 0 }, shaderSource: p };
...
var cl = O(() => {
  ...
  Dh = (i, e) => {
    ...
    executeProgram(e, o) {
      ...
      let r = Dh(e, t), n = this.session.programManager.getArtifact(r), s = n ? n.programInfo : typeof e.get == "function" ? e.get() : e, a = Cr(this.session.layoutStrategy, s.output.dims, s.output.textureType), u = this.createTextureData(a, s.output.type);
      ...
      if (!t) {
        ...
        if (o === 4) {
          ...
          if (a.length === 4) {
            ...
            if (a[1] * a[2] * a[3] % 4 !== 0) {
              let p = a[0], d = a[1] * a[2] * a[3], y = Math.ceil(d * 1 / 4) * 4, T = p * y;
        ...
        if (o === 2) {
          let n = pi(this.session.layoutStrategy, e.dims, 1, [], { reverseWH: true }), s = this.createTextureData(n, e.type, e.numberData, e, 1);
          t = this.pack(s);
    ...
    reshapePacked(e, o) {
      ...
      let r = fi(e.dims), n = fi(o), s = this.reshapePacked(e, r), a = this.run(el(this, s, n), [s]);
...
var ml = O(() => {
  ...
  ut();
  j();
  pl = { name: "BatchNormalization", inputNames: ["A", "Scale", "B", "Mean", "Variance"], inputTypes: [0, 0, 0, 0, 0] }, dl = (i, e, o) => ($h(e), [i.run({ ...pl, cacheHint: o.cacheKey, get: () => Lh(i, e, o) }, e)]), hl = (i) => {
  ...
  float process(int[${r}] indices) {
    ...
    return { ...pl, output: { dims: e[0].dims, type: e[0].type, textureType: 0 }, shaderSource: a };
  }, $h = (i) => {
...
var Pl = O(() => {
  ...
  qt = (i, e, o, t = e[0].type, r) => {
    ...
    return { name: o.name, inputNames: ["A", "B"], inputTypes: [n, n], cacheHint: r, get: () => jh(i, e, o, t) };
  }, jh = (i, e, o, t = e[0].type) => {
    let r = i.session.pack ? 2 : 0, n = !B.areEqual(e[0].dims, e[1].dims), s = e[0].dims, a = i.session.pack;
    if (n) {
      let f = kt.calcShape(e[0].dims, e[1].dims, false);
      if (!f) throw new Error("Can't perform binary op on the given tensors");
      s = f;
      let p = s.length, d = e[0].dims.length !== 0 ? e[0].dims.length : 1, y = e[1].dims.length !== 0 ? e[1].dims.length : 1, T = e[0].dims.length !== 0 ? "bcastIndices_A(indices, aindices);" : "aindices[0] = 0;", v = e[1].dims.length !== 0 ? "bcastIndices_B(indices, bindices);" : "bindices[0] = 0;", S = G(i.session.backend.glContext.version), L = a ? `
      ${o.body}
      void main() {
...
var kl = O(() => {
  ...
  pe();
  He();
  Jh = (i, e) => ({ name: "Concat (packed)", inputNames: Array.from({ length: i }, (o, t) => `X${t}`), inputTypes: Array(i).fill(2), cacheHint: e }), Yh = (i, e, o, t) => {
    ...
    for (let V = 1; V < o.length; V++) {
      ...
      for (let wt = 0; wt < r.length; wt++) if (wt === t) n[t] += lt[wt];
      else if (r[wt] !== lt[wt]) throw new Error("non concat dimensions must match");
    }
    let s = n.length, a = cr("coords", s), u = Bt(s), l = de(), f = o.map((V) => V.dims), p = ne(s), d = new Array(f.length - 1);
    d[0] = f[0][t];
    for (let V = 1; V < d.length; V++) d[V] = d[V - 1] + f[V][t];
    let y = p[t], T = p.slice(-2), v = p.join(), S = `if (${y} < ${d[0]}) {
        return getChannel(
            getX0(${v}), vec2(${T.join()}));
        }`;
    ...
    let L = d.length, P = d[d.length - 1];
    S += `
            return getChannel(
              getX${L}(${kn(p, y, P)}),
              vec2(${kn(T, y, P)}));`;
    let A = G(i.session.backend.glContext.version), M = `
          ${l}
...
var Nl = O(() => {
  ...
  j();
  kl();
  Bl = (i, e, o) => (om(e), i.session.pack && e[0].dims.length > 1 ? [i.run($l(i, e, o), e)] : [i.run(tm(i, e, o), e)]), Zh = (i, e) => ({ name: "Concat", inputNames: Array.from({ length: i }, (o, t) => `X${t}`), inputTypes: Array(i).fill(0), cacheHint: e }), Qh = (i, e, o, t) => {
    let r = o[0].dims.slice();
    if (t >= r.length || t < -1 * r.length) throw new Error("axis specified for concat doesn't match input dimensionality");
    t < 0 && (t = r.length + t);
    let n = r.slice(0);
    for (let y = 1; y < o.length; y++) {
      ...
      for (let v = 0; v < r.length; v++) if (v === t) n[t] += T[v];
      else if (r[v] !== T[v]) throw new Error("non concat dimensions must match");
    }
    let s = n.length, a = new Array(o.length), u = 0;
    for (let y = 0; y < a.length; ++y) u += o[y].dims[t], a[y] = u;
    let l = "";
    o.length < 5 ? l = Fl(a) : l = em(a);
    let f = rm(o.length, s), p = nm(a), d = `
        ${f}
        ${p}
        ${l}
...
`);
  }, Cl = (i) => W({ axis: i.attributes.getInt("axis") }), om = (i) => {
...
var xi = O(() => {
  ...
  }, Rl = (i, e) => [i.run(dt(i, e[0], im()), e)], Gl = (i, e) => [i.run(dt(i, e[0], am()), e)], Ml = (i, e) => [i.run(dt(i, e[0], sm()), e)], Ul = (i, e) => [i.run(dt(i, e[0], um()), e)], gi = (i, e, o) => [i.run(dt(i, e[0], hi(o.min, o.max), o.cacheKey), e)], Vl = (i) => W({ min: i.attributes.getFloat("min", Ve), max: i.attributes.getFloat("max", ze) }), zl = (i, e) => {
    let o = _m(i, e);
    return gi(i, [e[0]], o);
  }, _m = (i, e) => {
...
var lf = O(() => {
  ...
  Bn();
  qe();
  Sm = (i, e) => ({ name: "GroupedConv", inputNames: i ? ["X", "W", "Bias"] : ["X", "W"], inputTypes: i ? [0, 0, 0] : [0, 0], cacheHint: e }), Am = (i, e, o, t) => {
    ...
    let l = dr(s, a, t.dilations, t.pads, t.strides), f = G(i.session.backend.glContext.version), { activationFunction: p, applyActivation: d } = he(t), y = `
...
var cf = O(() => {
  ...
  j();
  He();
  Pm = (i) => ({ name: "Im2Col (packed)", inputNames: ["A"], inputTypes: [2], cacheHint: i }), Em = (i, e, o, t, r, n) => {
    let s = o.dims, a = t.dims, u = 2, l = 3, f = r.length, p = [a[1] * a[2] * a[3], r[2] * r[3]], d = a[2] * a[3], y = de(), T = G(i.session.backend.glContext.version), v = "";
...
function Lm(i, e, o) {
  let t = e[0].dims, r = e[1].dims, n = kt.calcShape(t, r, true);
  if (!n) throw new Error("Can't use matmul on the given tensors");
  let s = Bt(n.length), a = ne(), { activationFunction: u, applyActivation: l } = he(o), f = e.length > 2, p = f ? "value += getBiasForMatmul();" : "", d = f ? `${wi(s, a, e[2].dims, n, false)}` : "", y = n.length, T = t.length, v = r.length, S = t[t.length - 1], L = `
    ${u}
    ${d}
...
var Fn = O(() => {
  ...
  qe();
  vi();
  pf = (i, e, o) => ($m(e), i.session.pack ? [i.run(Cn(i, e, o), e)] : [i.run(Ti(e, o), e)]), df = (i) => pr(i.attributes), Dm = (i, e) => ({ name: "MatMul", inputNames: i ? ["A", "B", "Bias"] : ["A", "B"], inputTypes: i ? [0, 0, 0] : [0, 0], cacheHint: e });
  $m = (i) => {
...
function Fm(i, e, o, t) {
  ...
  let y = kt.getBroadcastDims(s, t), T = kt.getBroadcastDims(a, t), v = y.map((A) => `coords.${e[A + p]} = 0;`).join(`
...
var vi = O(() => {
  ...
  qe();
  Fn();
  km = (i, e) => ({ name: "MatMul (packed)", inputNames: i ? ["A", "B", "Bias"] : ["A", "B"], inputTypes: i ? [2, 2, 2] : [2, 2], cacheHint: e }), Bm = (i, e, o, t) => {
    let r = o.length > 2, n = r ? "value += getBiasForMatmul();" : "", s = o[0].dims, a = o[1].dims, u = kt.calcShape(s, a, true), l = !B.areEqual(o[0].dims, o[1].dims);
    if (!u) throw new Error("Can't use matmul on the given tensors");
    let f = s[s.length - 1], p = Math.ceil(f / 2), d = s.length, y = a.length, T = G(i.session.backend.glContext.version), v = Bt(u.length), S = u.length, L = ne(), { activationFunction: P, applyActivation: A } = he(t), M = r ? `${wi(v, L, o[2].dims, u, true)}` : "", V = l ? `${Fm(v, L, o, u)}` : "", lt = l ? "getAAtOutCoordsMatmul(i)" : `getA(${Cm(L, d)})`, wt = l ? "getBAtOutCoordsMatmul(i)" : `getB(${Nm(L, y)})`, et = l ? "" : `${v} rc =
    ...
    return { ...e, output: { dims: u, type: o[0].type, textureType: 2 }, shaderSource: Dt, hasMain: true };
  }, Cn = (i, e, o) => {
...
var mf = O(() => {
  ...
  vi();
  hf = (i, e, o) => {
    let t = e[0].dims, r = e[1].dims, n = dr(t, r, o.dilations, o.pads, o.strides), s = i.run(ff(i, e[0], e[1], n, o), [e[0]]), a = i.reshapePacked(e[1], [r[0], r[1] * r[2] * r[3]]), u = e.length === 3 ? [a, s, e[2]] : [a, s], l = i.run(Cn(i, u, o), u);
...
var _i = O(() => {
  "use strict";
  j();
  Rm = (i) => ({ name: "Im2Col", inputNames: ["X"], inputTypes: [0], cacheHint: i }), Gm = (i, e, o, t, r, n) => {
    let s = o.dims, a = t.dims, u = r.length, l = Ii(s, a, r, 4), f = `
        const int XC = ${s[1]};
        const int XH = ${s[2]};
        const int XW = ${s[3]};
...
var yf = O(() => {
  ...
  qe();
  _i();
  Mm = (i, e) => ({ name: "ConvDotProduct", inputNames: i ? ["Im2Col", "K", "B"] : ["Im2Col", "K"], inputTypes: i ? [0, 4, 0] : [0, 4], cacheKey: e.activationCacheKey }), Um = (i, e, o, t, r) => {
    let n = o[0].dims, s = o[1].dims, a = [s[0], Math.ceil(n[1] * s[2] * s[3] / 4)], u = Ii(n, s, t), [l, f] = i.calculateTextureWidthAndHeight(a, 4), p = B.computeStrides(u), [d, y] = i.calculateTextureWidthAndHeight(u, 4), T = t.length, v = o.length < 3 ? "0.0" : "_B(b)", S = Math.ceil(n[1] * s[2] * s[3] / 4), { activationFunction: L, applyActivation: P } = he(r), A = G(i.session.backend.glContext.version), M = `
${L}
...
var Bn = O(() => {
  ...
  Fn();
  dr = (i, e, o, t, r) => {
    let n = i[0], s = i.slice(2), a = s.length, u = e[0], f = e.slice(2).map((T, v) => T + (T - 1) * (o[v] - 1)), d = s.map((T, v) => T + t[v] + t[v + a]).map((T, v) => Math.floor((T - f[v] + r[v]) / r[v]));
    return [n, u].concat(...d);
  }, Oi = (i, e, o) => (qm(e, o), Vm(i, e, o)), Vm = (i, e, o) => {
    let t = Hm(o, e), r = i.session.pack, n = t.kernelShape[0] === 1 && t.kernelShape[1] === 1;
    return t.group > 1 ? [i.run(uf(i, e, t), e)] : n && r ? [zm(i, e, t)] : r && e[0].dims.length === 4 && e[0].dims[0] === 1 && !n ? [hf(i, e, t)] : [Wm(i, e, t)];
  }, zm = (i, e, o) => {
    let t = e[0].dims, r = e[1].dims, n = dr(t, r, o.dilations, o.pads, o.strides), s = i.reshapeUnpacked(e[0], [t[1], t[2] * t[3]]), a = i.reshapeUnpacked(e[1], [r[0], r[1]]), u = e.length > 2 ? [a, s, e[2]] : [a, s], l = i.run(Ti(u, o), u);
    return i.reshapeUnpacked(l, n);
  }, Wm = (i, e, o) => {
    let t = e[0].dims, r = e[1].dims, n = dr(t, r, o.dilations, o.pads, o.strides), s = i.run(bf(i, e[0], e[1], n, o), [e[0]]), a = e.length === 3 ? [s, e[1], e[2]] : [s, e[1]];
...
var wf = O(() => {
  ...
  jm = (i, e, o, t, r, n) => (i - 1) * e + o + (t - 1) * r + 1 - n, Xm = (i, e, o, t, r) => {
    ...
    for (let f = 0; f < u; ++f) {
      let p = l ? i[f + 2] * n[f] : a[f], d = jm(i[f + 2], n[f], r[f], e[f], o[f], p);
      Xm(d, t, r, f, f + u), l && a.push(n[f] * (i[f + 2] - 1) + s[f] + (e[f] - 1) * o[f] + 1 - r[f] - r[f + u]);
    }
  }, xf = (i, e, o) => (rb(e, o), Jm(i, e, o)), Jm = (i, e, o) => {
    let t = eb(o, e);
    return [tb(i, e, t)];
  }, Ym = (i, e) => ({ name: "ConvTranspose", inputNames: i ? ["X", "W", "B"] : ["X", "W"], inputTypes: i ? [0, 0, 0] : [0, 0], cacheHint: e }), Zm = (i, e, o, t) => {
    let n = e.length > 2 ? "getB(output_channel)" : "0.0", s = e[0].dims, a = e[1].dims, u = a[1], l = a[0] / t.group, f = [e[0].dims[0], e[1].dims[1] * t.group, ...t.outputShape], p = G(i.session.backend.glContext.version), { activationFunction: d, applyActivation: y } = he(t), T = `
  const ivec2 strides = ivec2(${t.strides[0]}, ${t.strides[1]});
  const ivec2 pads = ivec2(${t.pads[0]}, ${t.pads[1]});
  ${d}
  void main() {
...
`;
    return { ...o, output: { dims: f, type: e[0].type, textureType: 0 }, shaderSource: T, hasMain: true };
  }, Qm = (i, e, o) => {
...
var Nn = O(() => {
  ...
  Y();
  j();
  vf = { name: "Transpose", inputNames: ["A"], inputTypes: [0] }, je = (i, e, o) => (ab(e), [i.run({ ...vf, cacheHint: o.cacheKey, get: () => nb(i, e[0], o.perm) }, e)]), If = (i) => W({ perm: i.attributes.getInts("perm", []) }), nb = (i, e, o) => {
    let t = e.dims;
    o = _f(t, o);
    let r = ob(t, o), n = t.length, s = `
      ${ib("perm", o, n)}
    ...
    return { ...vf, output: { dims: r, type: e.type, textureType: 0 }, shaderSource: s };
...
var Af = O(() => {
  ...
  Of = (i, e, o) => {
    sb(e);
    let t = o.blocksize, r = t * t, n = o.mode === "DCR" ? [0, 3, 4, 1, 5, 2] : [0, 1, 4, 2, 5, 3], s = o.mode === "DCR" ? [e[0].dims[0], t, t, e[0].dims[1] / r, e[0].dims[2], e[0].dims[3]] : [e[0].dims[0], e[0].dims[1] / r, t, t, e[0].dims[2], e[0].dims[3]], a = i.reshapeUnpacked(e[0], s), u = { perm: n, cacheKey: `${n}` }, [l] = je(i, [a], u), f = [e[0].dims[0], e[0].dims[1] / r, e[0].dims[2] * t, e[0].dims[3] * t];
...
var kf = O(() => {
  ...
  Y();
  j();
  Lf = (i, e, o) => (pb(e, o.axis), [i.run(cb(i, e, o), e)]), $f = (i) => W({ axis: i.attributes.getInt("axis", 0) }), lb = { name: "Gather", inputNames: ["A", "B"], inputTypes: [0, 0] }, fb = (i, e, o, t) => {
...
var Nf = O(() => {
  ...
  Ai = (i, e, o) => (mb(e, o), [i.run(db(e, o), e)]), Bf = (i, e) => {
  ...
  }, Ff = (i) => Bf(i, false), Cf = (i) => Bf(i, true), db = (i, e) => {
    let o = { name: "Gemm", inputNames: i.length === 3 ? ["A", "B", "C"] : ["A", "B"], inputTypes: i.length === 3 ? [0, 0, 0] : [0, 0], key: e.cacheKey };
    return { ...o, get: () => hb(o, i, e) };
  }, hb = (i, e, o) => {
    let t = e[0].dims.slice(), r = e[1].dims.slice(), [n, s] = Sn.getShapeOfGemmResult(t, o.transA, r, o.transB, e.length === 3 ? e[2].dims : void 0), a = [n, s];
    ...
    let f = a.length, p = e.length === 3 ? `int c[${e[2].dims.length}];` : "", d = e.length === 3 ? "bcastIndices_C(indices, c);" : "", y = e.length === 3 ? "value += beta * _C(c);" : "", T = `
    ...
    return { ...i, output: { dims: a, type: e[0].type, textureType: 0 }, variables: [{ name: "alpha", type: "float", data: o.alpha }, { name: "beta", type: "float", data: o.beta }], shaderSource: T };
  }, mb = (i, e) => {
...
var Mf = O(() => {
  ...
  }, bb = { name: "ImageScaler", inputNames: ["X"], inputTypes: [0] }, gb = (i, e, o, t) => {
...
var Wf = O(() => {
  ...
  }, zf = (i) => i.attributes.getFloat("epsilon", 1e-5), Uf = { name: "InstanceNormalization_MeanAndVariance", inputNames: ["X"], inputTypes: [0] }, wb = (i, e) => {
  ...
  }, vb = (i) => ({ ...Uf, get: () => wb(Uf, i) }), Ib = { name: "InstanceNormalization_ComputeOutput", inputNames: ["X", "MeanAndVariance", "Scale", "B"], inputTypes: [0, 4, 0, 0] }, _b = (i, e, o, t, r) => {
...
function Ab(i, e) {
  ...
  return { ...jf, cacheHint: e.cacheKey, output: { dims: i[0].dims, type: i[0].type, textureType: 0 }, shaderSource: l };
}
function Pb(i, e) {
...
var Zf = O(() => {
  ...
  ut();
  j();
  Db = { name: "Pad", inputNames: ["A"], inputTypes: [0] }, Pi = (i, e, o) => (kb(e), [i.run({ ...Db, cacheHint: o.cacheKey, get: () => $b(i, e[0], o) }, e)]), Kf = (i) => {
...
var cc = O(() => {
  ...
  tc = (i, e, o) => {
    ...
    return [i.run({ ...t, get: () => rc(e, t, false, o) }, e)];
  }, ec = (i) => {
    ...
    let [r, n] = uc(i, t, o), s = B.size(r.kernelShape), a = "value += _X(x);", u = "";
    ...
    return [i.run({ ...t, get: () => rc(e, t, true, o) }, e)];
  }, oc = (i) => {
    ...
    return [i.run({ ...t, get: () => sc(e, t, false, o) }, e)];
  }, ac = (i) => {
    ...
    let l = Ue.computePoolOutputShape(o, t, s, a, n, u, e.autoPad), f = Object.assign({}, e);
  ...
  }, Gb = { autoPad: "", ceilMode: 0, countIncludePad: false, kernelShape: [], strides: [], pads: [], storageOrder: 0, dilations: [], cacheKey: "" }, Qf = { name: "GlobalMaxPool", inputNames: ["X"], inputTypes: [0] }, lc = (i, e) => (Gn(e), [i.run({ ...Qf, get: () => sc(e, Qf, true, Gb) }, e)]), Gn = (i) => {
    ...
    if (e.kernelShape.length <= 2) {
      ...
      if (u + l !== 0 ? p = `
        ...
        let v = e.kernelShape[e.kernelShape.length - 2], S = e.strides[e.strides.length - 2], L = e.pads[e.pads.length / 2 - 2], P = e.pads[e.pads.length - 2], A = i[n - 2];
        L + P !== 0 ? d = `
            for (int j = 0; j < ${v}; j++) {
      ...
      let s = B.size(e.kernelShape), a = B.computeStrides(e.kernelShape), u = a.length, l = e.pads.length, f = Mb(u), p = Rn(i, "inputDims"), d = Rn(e.pads, "pads"), y = Rn(a, "kernelStrides"), T = Rn(e.strides, "strides"), v = e.pads.reduce((P, A) => P + A), S = "";
      return v ? S = `
...
var xc = O(() => {
  ...
  Xe = (i, e, o, t, r) => {
    ...
    return [i.run({ ...n, cacheHint: o.cacheKey, get: () => Ub(i, e, o, t, r, n) }, e)];
  }, Ee = (i) => {
...
var Li = O(() => {
  ...
  ut();
  j();
  vc = { name: "Upsample", inputNames: ["X"], inputTypes: [0] }, Ei = (i, e, o) => (Di(e, o), [i.run({ ...vc, cacheHint: o.cacheKey, get: () => zb(i, e, o) }, e)]), Ic = (i) => Gr(i, 7), _c = (i) => Gr(i, 9), Gr = (i, e) => {
    ...
    let d = e < 11 ? true : t === "nearest" && s === "asymmetric" && l === "floor", y = 0, T = 0, v = 0;
    return e > 10 ? i.inputs.length > 2 ? (y = 1, T = 2, v = 3) : (T = 1, v = 2) : e === 9 && (T = 1), W({ opset: e, isResize: o, mode: t, scales: r, extrapolationValue: n, coordinateTransformMode: s, useExtrapolation: u, needRoiInput: a, nearestMode: l, cubicCoefficientA: f, excludeOutside: p, useNearest2xOptimization: d, roiInputIdx: y, scalesInputIdx: T, sizesInputIdx: v });
  }, zb = (i, e, o) => {
    ...
    return { ...vc, output: { dims: s, type: e[0].type, textureType: 0 }, shaderSource: T, variables: [{ name: "scales", type: "int", arrayLength: o.scales.length, data: o.scales.map((v) => Math.ceil(v)) }] };
  }, Di = (i, e) => {
...
var Ac = O(() => {
  ...
  He();
  Li();
  $i = { name: "Resize", inputNames: ["A"], inputTypes: [2] }, ki = (i, e, o) => (Di(e, o), [i.run({ ...$i, cacheHint: o.cacheKey, get: () => Wb(i, e, o) }, e)]), Oc = (i) => Gr(i, 10), Sc = (i) => Gr(i, 11), Wb = (i, e, o) => {
    let t = G(i.session.backend.glContext.version), [r, n] = Hb(e, o);
    if (r.every((A) => A === 1) && o.coordinateTransformMode !== "tf_crop_and_resize") return { ...$i, output: { dims: n, type: e[0].type, textureType: 2 }, hasMain: true, shaderSource: `void main() {
    ...
    let p = f[a - 2], d = f[a - 1], y = r[a - 2], T = r[a - 1], v = "";
    ...
    let S = Bt(a), L = de(), P = `
        ...
        `;
    return { ...$i, output: { dims: n, type: e[0].type, textureType: 2 }, hasMain: true, shaderSource: P };
  }, Hb = (i, e) => {
...
var Bc = O(() => {
  ...
  Y();
  j();
  Bi = { name: "Slice", inputNames: ["A"], inputTypes: [0] }, Dc = (i, e, o) => (Kb(e), [i.run({ ...Bi, cacheHint: o.cacheKey, get: () => $c(i, e[0], o) }, e)]), Lc = (i) => {
    ...
    return { ...Bi, output: { dims: a, type: e.type, textureType: 0 }, shaderSource: f };
  }, Kb = (i) => {
    ...
    Yb(e);
    let o = Jb(i, e);
    return [i.run({ ...Bi, cacheHint: o.cacheKey, get: () => $c(i, e[0], o) }, [e[0]])];
  }, Jb = (i, e) => {
...
var Wc = O(() => {
  ...
  j();
  Nn();
  Fc = { name: "SoftmaxComputeMax", inputNames: ["A"], inputTypes: [0] }, Cc = { name: "SoftmaxComputeScale", inputNames: ["A", "Max"], inputTypes: [0, 0] }, Nc = { name: "SoftMax", inputNames: ["A", "Max", "Norm"], inputTypes: [0, 0, 0] }, Rc = (i, e, o) => {
    zc(e);
    let t = e[0].dims.slice(), r = B.normalizeAxis(o.axis, t.length), n = B.sizeToDimension(t, r), s = B.sizeFromDimension(t, r);
    return Vc(i, e, o, n, s);
  }, Gc = (i) => W({ axis: i.attributes.getInt("axis", 1) }), Mc = (i) => W({ axis: i.attributes.getInt("axis", -1) }), Uc = (i, e, o) => {
    zc(e);
    let t = e[0].dims.slice(), r = B.normalizeAxis(o.axis, t.length), n = t.length, s = r !== n - 1, a = [], u = [], l = [], f;
    s && (u = Array.from({ length: n }).map((T, v) => v), u[r] = n - 1, u[n - 1] = r, u.map((T) => a.push(t[T])), f = W({ perm: u }), l = je(i, e, f));
    let p = s ? B.sizeToDimension(a, n - 1) : B.sizeToDimension(t, n - 1), d = s ? B.sizeFromDimension(a, n - 1) : B.sizeFromDimension(t, n - 1), y = Vc(i, s ? l : e, o, p, d);
    return s ? je(i, y, f) : y;
  }, Vc = (i, e, o, t, r) => {
    let n = Zb(i, e[0], t, r, [t]), s = i.run({ ...Fc, cacheHint: o.cacheKey, get: () => n }, e), a = Qb(i, e[0], t, r, n.output.dims, [t]), u = i.run({ ...Cc, cacheHint: o.cacheKey, get: () => a }, [e[0], s]), l = tg(i, e[0], t, r, n.output.dims, a.output.dims);
    return [i.run({ ...Nc, cacheHint: o.cacheKey, get: () => l }, [e[0], s, u])];
  }, Zb = (i, e, o, t, r) => {
    ...
    return { ...Fc, output: { dims: r, type: e.type, textureType: 0 }, shaderSource: l };
  }, Qb = (i, e, o, t, r, n) => {
    ...
    return { ...Cc, output: { dims: n, type: e.type, textureType: 0 }, shaderSource: f };
  }, tg = (i, e, o, t, r, n) => {
    ...
    return { ...Nc, output: { dims: e.dims, type: e.type, textureType: 0 }, shaderSource: l };
  }, zc = (i) => {
...
var Xc = O(() => {
  ...
  Y();
  j();
  Hc = { name: "Split", inputNames: ["A"], inputTypes: [0] }, qc = (i, e, o) => {
    ng(e);
    let t = B.normalizeAxis(o.axis, e[0].dims.length), r = eg(i, e, t, o), n = [];
    for (let s = 0; s < r; ++s) n.push(i.run({ ...Hc, cacheHint: `${o.cacheKey};${s}`, get: () => rg(i, e[0], o, t, s) }, e));
    return n;
  }, jc = (i) => {
    ...
    `;
    return { ...Hc, cacheHint: `${o.cacheKey}:${r}`, output: { dims: u, type: e.type, textureType: 0 }, shaderSource: f };
  }, ng = (i) => {
...
function up(i) {
  ...
  for (let t in e) {
    let r = pg.replace("__FUNC__", t), n = new RegExp(r, "gm");
    for (; (o = n.exec(i)) !== null; ) {
...
function hr(i, e) {
  let o = [], t = [], r = e != null && Array.isArray(e) && e.length === 0, n = e == null || r ? null : dg(e, i).sort(), s = 0;
  ...
  Vn = class extends Ht {
    ...
    getInputsSamplingSnippets() {
      ...
      return this.context.programInfo.inputNames.forEach((r, n) => {
        let s = this.context.inputTextureLayouts[n], a = An(r);
    ...
    getPackedSamplerAtOutputCoords(o, t, r, n) {
      let s = t.unpackedShape, a = r.unpackedShape, l = An(n), f = s.length, p = a.length, d = kt.getBroadcastDims(s, a), y = Bt(p), T = p - f, v, S = ne();
      f === 0 ? v = "" : p < 2 && d.length >= 1 ? v = "coords = 0;" : v = d.map((Dt) => `coords.${S[Dt + T]} = 0;`).join(`
`);
      let L = "";
      p < 2 && f > 0 ? L = "coords" : L = s.map((Dt, _t) => `coords.${S[_t + T]}`).join(", ");
      let P = "return outputValue;", M = B.size(s) === 1, lt = B.size(a) === 1;
      if (f === 1 && !M && !lt) P = `
    ...
    getUnpackedSamplerAtOutputCoords(o, t, r, n) {
      let s = [r.width, r.height], a = [t.width, t.height], u = t.unpackedShape.length, l = r.unpackedShape.length, f = t.unpackedShape, p = r.unpackedShape, d = An(n);
      ...
      let y = Bt(l), T = kt.getBroadcastDims(f, p), v = l - u, S, L = ne();
...
`);
    ...
    getPackedSampler3D(o, t, r) {
      ...
      if (n[0] === 1) {
        let v = n.slice(1), S = [1, 2], L = lr(n, v), P = ["b", "row", "col"], A = JSON.parse(JSON.stringify(r));
        A.unpackedShape = L;
        let M = this.getPackedSamplerFromInput(o, t, A), lt = `${M.routineBody}
      vec4 ${o}(int b, int row, int col) {
        return ${o}(${fr(P, S)});
      } `;
        return new k(lt, M.dependencies);
      }
      let l = a[0], f = a[1], p = Math.ceil(n[2] / 2), d = p * Math.ceil(n[1] / 2), T = `vec4 ${o}(int b, int row, int col) {
    ...
    getPackedSamplerND(o, t, r) {
      let n = r.unpackedShape, s = n.length, a = [r.width, r.height], u = G(this.context.glContext.version), l = [a[0], a[1]], f = l[1], p = l[0], d = Math.ceil(n[s - 1] / 2), y = d * Math.ceil(n[s - 2] / 2), T = "int b, int row, int col", v = `b * ${y} + (row / 2) * ${d} + (col / 2)`;
    ...
    getUnpackedSampler2D(o, t, r) {
      ...
      if (l.length < n.length) {
        let y = lr(n, l), T = JSON.parse(JSON.stringify(r));
        T.unpackedShape = y;
        let v = ["col", "row"], S = `
    ...
    getUnpackedSampler3D(o, t, r) {
      ...
      if (f.length < n.length) {
        let T = lr(n, f), v = ["batch", "col", "row"], S = JSON.parse(JSON.stringify(r));
        S.unpackedShape = T;
        let L = this.getUnpackedSamplerFromInput(o, t, S), P = l.reverse(), A = `
          ${L.routineBody}
    ...
    getUnpackedSampler6D(o, t, r) {
      ...
      if (p.length < n.length) {
        let S = lr(n, p), L = ["row", "col", "depth", "depth2", "depth3", "depth4"], P = JSON.parse(JSON.stringify(r));
        P.unpackedShape = S;
        let A = `
...
var bp = O(() => {
  ...
  hp();
  mp();
  Ri = { encoding: zn, fragcolor: Wn, vec: qn, shapeUtils: Hn, coordinates: Vn };
});
var jn;
var gp = O(() => {
  "use strict";
  ...
  Xn = class {
    ...
    build(e, o, t) {
      return this.profiler.event("backend", "ProgramManager.build", () => {
        let r = new jn(this.glContext, e, o, t), n = r.preprocess(), s = this.compile(n);
...
${e}`);
    ...
    async addItemToPoll(e, o) {
      this.itemsToPoll.push({ isDoneFn: e, resolveFn: o }), !(this.itemsToPoll.length > 1) && await li(() => (this.pollItems(), this.itemsToPoll.length === 0));
    }
  };
});
function Gi(i) {
...
var _p = O(() => {
  ...
  Yn = class {
    ...
    initialize() {
      try {
        return this.glContext = Gi(this.contextId), typeof this.matmulMaxBatchSize != "number" && (this.matmulMaxBatchSize = 16), typeof this.textureCacheMode != "string" && (this.textureCacheMode = "full"), typeof this.pack != "boolean" && (this.pack = false), typeof this.async != "boolean" && (this.async = false), tt.setWithEnv(z), z.webgl.context || Object.defineProperty(z.webgl, "context", { value: this.glContext.gl }), tt.verbose("WebGLBackend", `Created WebGLContext: ${typeof this.glContext} with matmulMaxBatchSize: ${this.matmulMaxBatchSize}; textureCacheMode: ${this.textureCacheMode}; pack: ${this.pack}; async: ${this.async}.`), true;
      } catch (e) {
...
var Sp = O(() => {
  "use strict";
  _p();
  Op = /* @__PURE__ */ new Map(), xg = { webgl: new Yn() };
});
var Ui;
var Zn;
var Ap = O(() => {
...
var Ep = O(() => {
  ...
  We();
  Y();
  Qn = F.experimental.fbs, Wi = { from: (i, e) => new Vi(i, e) }, me = class {
    constructor(e) {
    ...
    constructor(e, o) {
      e instanceof zi.onnx.NodeProto ? (this.name = e.name, this.opType = e.opType, this.attributes = new Ur(e.attribute)) : e instanceof Qn.Node && (this.name = o ?? e.name(), this.opType = e.opType(), this.attributes = new Ur(At.tensorAttributesFromORTFormat(e))), this.inputs = [], this.outputs = [], this.executeNode = true;
    }
  }, Vi = class {
    constructor(e, o) {
    ...
    buildGraphFromOnnxFormat(e) {
      ...
      for (let n of e.initializer) {
        ...
        if (s === void 0) {
          let a = new me();
          a.type = { shape: { dims: At.tensorDimsFromProto(n.dims) }, tensorType: At.tensorDataTypeFromProto(n.dataType) }, s = this._allData.push(a) - 1, o.set(n.name, s);
        }
    ...
    buildGraphFromOrtFormat(e) {
      ...
      for (let n = 0; n < e.inputsLength(); n++) {
        ...
        for (let a = 0; a < e.nodeArgsLength(); a++) if (e.nodeArgs(a)?.name() === s) {
          ...
          let f = e.nodeArgs(a).type().value(new Qn.TensorTypeAndShape()), p = At.tensorDataTypeFromProto(f.elemType()), d = f.shape(), y = [];
      ...
      for (let n = 0; n < e.initializersLength(); n++) {
        ...
        if (a === void 0) {
          let u = new me(), l = At.tensorDimsFromORTFormat(s), f = At.tensorDataTypeFromProto(s.dataType());
...
as ORT format: ${n}`);
    ...
    loadFromOnnxFormat(e, o) {
      ...
      this._opsets = t.opsetImport.map((n) => ({ domain: n.domain, version: Rt.longToNumber(n.version) })), this._graph = Wi.from(t.graph, o);
    }
    loadFromOrtFormat(e, o) {
      let t = new w.ByteBuffer(e), r = vg.InferenceSession.getRootAsInferenceSession(t).model();
      ...
      for (let s = 0; s < r.opsetImportLength(); s++) {
        ...
        this._opsets.push({ domain: a?.domain(), version: Rt.longToNumber(a.version()) });
      }
...
var $p = O(() => {
  ...
  ro = class {
    constructor(e = {}) {
      this._initialized = false, this.backendHint = e.backendHint, this.profiler = xn.create(e.profiler), this.context = { profiler: this.profiler, graphInputTypes: [], graphInputDims: [] };
    }
    get inputNames() {
      ...
      switch (e) {
        ...
        case "run": {
          ...
          fo(t, r, n, s, new Array(s.length).fill(null), a).then((u) => {
            u.some((l) => l[3] !== "cpu") ? postMessage({ type: e, err: "Proxy does not support non-cpu tensor location." }) : postMessage({ type: e, out: u }, po([...n, ...u]));
          }, (u) => {
...
var Vp = O(() => {
  ...
  Mp = (ji = import.meta.url, async function(i = {}) {
    ...
    u.mountExternalData = (c, m) => {
      c.startsWith("./") && (c = c.substring(2)), (u.Ua || (u.Ua = /* @__PURE__ */ new Map())).set(c, m);
    }, u.unmountExternalData = () => {
      delete u.Ua;
    };
    var y, T, v = globalThis.SharedArrayBuffer ?? new WebAssembly.Memory({ initial: 0, maximum: 0, shared: true }).buffer.constructor, S = Object.assign({}, u), L = "./this.program", P = (c, m) => {
      throw m;
    }, A = "";
    ...
    var M, V = console.log.bind(console), lt = console.error.bind(console), wt = V, et = lt;
      ...
      let c = function(m) {
        try {
          ...
          if (x === "load") {
            ...
            $o(g.pthread_ptr, 0, 0, 1, 0, 0), Ao(g.pthread_ptr), md(), aa(), _t || (_t = true);
            try {
    ...
    var C, Kr, we, oe, be, Z, ge, ie, ae, se = false;
    function it() {
      var c = C.buffer;
    ...
    var Lt = [], Jr = [], Ye = [], ue = 0, Ze = null, De = null;
    function le() {
    ...
    function Mt(c) {
      throw et(c = "Aborted(" + c + ")"), se = true, we = 1, c = new WebAssembly.RuntimeError(c + ". Build with -sASSERTIONS for more info."), a(c), c;
    }
    var ve, gt = (c) => c.startsWith("data:application/octet-stream;base64,"), Ot = (c) => c.startsWith("file://");
    function Jt(c) {
    ...
    function wr(c, m, g) {
      return function(x) {
        if (!M && (f || p)) {
          if (typeof fetch == "function" && !Ot(x)) return fetch(x, { credentials: "same-origin" }).then((I) => {
    ...
    function Yr() {
      return { a: { j: hd, b: yd, E: ca, g: ha, V: ma, A: ya, C: xa, W: Ta, T: wa, L: va, S: Ia, o: _a, B: Oa, y: Sa, U: Aa, z: Pa, _: xd, Z: Td, P: wd, w: vd, F: Id, k: _d, O: Ao, Y: Od, I: Sd, J: Ad, K: Pd, G: La, H: $a, v: Ed, q: Dd, l: Ld, p: $d, e: kd, X: Bd, x: Fd, d: ka, f: Cd, i: Nd, u: Rd, t: Gd, s: Md, Q: Ca, R: Na, D: So, h: Ra, n: Ga, M: Ma, m: Ua, a: C, r: Oo, N: Wa, c: zd } };
    }
    var ea = { 837620: (c, m, g, x, I) => {
    ...
    var sa = (c) => new Promise((m) => {
      c.onmessage = (I) => {
        ...
        } else E === "checkMailbox" ? tn() : E === "spawnThread" ? ra(I) : E === "cleanupThread" ? ia(fe[I.thread]) : E === "killThread" ? (I = I.thread, E = fe[I], delete fe[I], _o(E), ko(I), Le.splice(Le.indexOf(E), 1), E.Ra = 0) : E === "cancelThread" ? fe[I.thread].postMessage({ cmd: "cancel" }) : E === "loaded" ? (c.loaded = true, m(c)) : E === "alert" ? alert(`Thread ${I.threadId}: ${I.text}`) : I.target === "setimmediate" ? c.postMessage(I) : E === "callHandler" ? u[I.handler](...I.args) : E && et(`worker sent an unknown command ${E}`);
      }, c.onerror = (I) => {
        throw et(`worker sent an error! ${I.filename}:${I.lineno}: ${I.message}`), I;
      };
      var g, x = [];
      for (g of []) u.hasOwnProperty(g) && x.push(g);
      c.postMessage({ cmd: "load", handlers: x, wasmMemory: C, wasmModule: Kr });
    });
    function ua() {
    ...
    function ca(c, m, g, x) {
      ...
      var I = [];
      return d && I.length === 0 ? fa(c, m, g, x) : (c = { cb: g, Ra: c, ab: x, ib: I }, d ? (c.Za = "spawnThread", postMessage(c, I), 0) : ra(c));
    }
    var pa = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, da = (c, m, g) => {
      var x = (m >>>= 0) + g;
      for (g = m; c[g] && !(g >= x); ) ++g;
    ...
    function vd(c, m) {
      (c >>>= 0) == m >>> 0 ? setTimeout(tn) : d ? postMessage({ targetThread: c, cmd: "checkMailbox" }) : (c = fe[c]) && c.postMessage({ cmd: "checkMailbox" });
    }
    var Po = [];
    ...
    function _d(c) {
      c >>>= 0, d ? postMessage({ cmd: "cleanupThread", thread: c }) : ia(fe[c]);
    }
    function Od(c) {
    }
    function Sd(c, m) {
    ...
    var Qe = (c) => c % 4 == 0 && (c % 100 != 0 || c % 400 == 0), Ea = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335], Da = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
    function Ad(c, m) {
    ...
    function Ed(c, m, g, x) {
      ...
      var at = R.getTimezoneOffset(), Pt = Math.max(I, at);
      r()[c >>> 2 >>> 0] = 60 * Pt, t()[m >>> 2 >>> 0] = +(I != at), E = (c = ($t) => $t.toLocaleTimeString(void 0, { hour12: false, timeZoneName: "short" }).split(" ")[1])(E), R = c(R), at < I ? (_r(E, g, 17), _r(R, x, 17)) : (_r(E, x, 17), _r(R, g, 17));
    }
    var Eo = [];
    function Dd(c, m, g) {
    ...
    var en = () => (Mt("Cannot use convertFrameToPC (needed by __builtin_return_address) without -sUSE_OFFSET_CONVERTER"), 0), tr = {}, Ba = (c) => {
      c.forEach((m) => {
        var g = en();
        g && (tr[g] = m);
      });
...
`))[0] == "Error" && x.shift(), Ba(x);
    ...
    function Ua(c, m, g, x) {
      ...
      for (var I = 0, E = 0; E < g; E++) {
        var R = r()[m >>> 2 >>> 0], at = r()[m + 4 >>> 2 >>> 0];
        m += 8;
        for (var Pt = 0; Pt < at; Pt++) {
          var $t = o()[R + Pt >>> 0], Ft = Ud[c];
    ...
    var Va = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], za = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], Vd = (c, m) => {
      e().set(c, m >>> 0);
    };
    function Wa(c, m, g, x) {
      function I(_, Q, ft) {
      ...
      function Pt(_) {
        ...
        for (_ = new Date(new Date(_.Ta + 1900, 0, 1).getTime()); 0 < Q; ) {
          var ft = _.getMonth(), $e = (Qe(_.getFullYear()) ? Va : za)[ft];
          if (!(Q > $e - _.getDate())) {
      ...
      c >>>= 0, m >>>= 0, g >>>= 0, x >>>= 0;
      var $t = r()[x + 40 >>> 2 >>> 0];
      for (var Ft in x = { gb: t()[x >>> 2 >>> 0], fb: t()[x + 4 >>> 2 >>> 0], Va: t()[x + 8 >>> 2 >>> 0], Ya: t()[x + 12 >>> 2 >>> 0], Wa: t()[x + 16 >>> 2 >>> 0], Ta: t()[x + 20 >>> 2 >>> 0], Qa: t()[x + 24 >>> 2 >>> 0], Sa: t()[x + 28 >>> 2 >>> 0], ob: t()[x + 32 >>> 2 >>> 0], eb: t()[x + 36 >>> 2 >>> 0], hb: $t ? Ir($t) : "" }, g = Ir(g), $t = { "%c": "%a %b %d %H:%M:%S %Y", "%D": "%m/%d/%y", "%F": "%Y-%m-%d", "%h": "%b", "%r": "%I:%M:%S %p", "%R": "%H:%M", "%T": "%H:%M:%S", "%x": "%m/%d/%y", "%X": "%H:%M:%S", "%Ec": "%c", "%EC": "%C", "%Ex": "%m/%d/%y", "%EX": "%H:%M:%S", "%Ey": "%y", "%EY": "%Y", "%Od": "%d", "%Oe": "%e", "%OH": "%H", "%OI": "%I", "%Om": "%m", "%OM": "%M", "%OS": "%S", "%Ou": "%u", "%OU": "%U", "%OV": "%V", "%Ow": "%w", "%OW": "%W", "%Oy": "%y" }) g = g.replace(new RegExp(Ft, "g"), $t[Ft]);
      var Ja = "Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" "), Ya = "January February March April May June July August September October November December".split(" ");
      for (Ft in $t = { "%a": (_) => Ja[_.Qa].substring(0, 3), "%A": (_) => Ja[_.Qa], "%b": (_) => Ya[_.Wa].substring(0, 3), "%B": (_) => Ya[_.Wa], "%C": (_) => E((_.Ta + 1900) / 100 | 0, 2), "%d": (_) => E(_.Ya, 2), "%e": (_) => I(_.Ya, 2, " "), "%g": (_) => Pt(_).toString().substring(2), "%G": Pt, "%H": (_) => E(_.Va, 2), "%I": (_) => ((_ = _.Va) == 0 ? _ = 12 : 12 < _ && (_ -= 12), E(_, 2)), "%j": (_) => {
...
`, "%p": (_) => 0 <= _.Va && 12 > _.Va ? "AM" : "PM", "%S": (_) => E(_.gb, 2), "%t": () => "	", "%u": (_) => _.Qa || 7, "%U": (_) => E(Math.floor((_.Sa + 7 - _.Qa) / 7), 2), "%V": (_) => {
      ...
      }, "%Z": (_) => _.hb, "%%": () => "%" }, g = g.replace(/%%/g, "\0\0"), $t) g.includes(Ft) && (g = g.replace(new RegExp(Ft, "g"), $t[Ft](x)));
    ...
    var Wd = [Oo, na, fa, ha, ma, ya, xa, Ta, wa, va, Ia, _a, Oa, Sa, Aa, Pa, La, $a, Ca, Na, Ra, Ga, Ma, Ua], U = function() {
      function c(g, x) {
        return U = g.exports, U = function() {
          var I = U, E = (at) => () => at() >>> 0, R = (at) => (Pt) => at(Pt) >>> 0;
      ...
      return ve || (ve = u.locateFile ? gt("ort-wasm-simd-threaded.wasm") ? "ort-wasm-simd-threaded.wasm" : u.locateFile ? u.locateFile("ort-wasm-simd-threaded.wasm", A) : A + "ort-wasm-simd-threaded.wasm" : new URL("ort-wasm-simd-threaded.wasm", import.meta.url).href), function(g, x) {
        var I = ve;
        return M || typeof WebAssembly.instantiateStreaming != "function" || gt(I) || Ot(I) || typeof fetch != "function" ? wr(I, g, x) : fetch(I, { credentials: "same-origin" }).then((E) => WebAssembly.instantiateStreaming(E, g).then(x, function(R) {
    ...
    var rn, $o = (c, m, g, x, I, E) => ($o = U.Ga)(c, m, g, x, I, E), Ha = () => (Ha = U.Ha)(), qa = (c, m, g, x, I) => (qa = U.Ia)(c, m, g, x, I), ko = (c) => (ko = U.Ja)(c), nn = (c) => (nn = U.Ka)(c), ja = () => (ja = U.La)(), Xa = (c, m) => (Xa = U.Ma)(c, m), on = (c) => (on = U.Na)(c), Bo = (c) => (Bo = U.Oa)(c), Fo = () => (Fo = U.Pa)();
    function Ka() {
      0 < ue || (d ? (s(u), d || Zr(Jr), startWorker(u)) : (Zr(Lt), 0 < ue || rn || (rn = true, u.calledRun = true, se || (d || Zr(Jr), s(u), d || Zr(Ye)))));
    }
...
var Vr = O(() => {
  "use strict";
  oo();
  br = false ? void 0 : import.meta.url ?? (typeof document < "u" ? document.currentScript?.src : typeof self < "u" ? self.location?.href : void 0), Sg = typeof location > "u" ? void 0 : location.origin, Ag = (i, e) => {
    try {
      let o = e ?? br;
...
var Ke = O(() => {
  ...
  Ki = false, ho = false, qp = false, Dg = () => {
    ...
    let r = i.wasmPaths, n = typeof r == "string" ? r : void 0, s = r?.mjs, a = s?.href ?? s, u = r?.wasm, l = u?.href ?? u, f = i.wasmBinary, [p, d] = await Hp(a, n, o > 1), y = false, T = [];
    if (e > 0 && T.push(new Promise((v) => {
      ...
      f ? L.wasmBinary = f : (l || n) && (L.locateFile = (P, A) => l ?? (n ?? A) + P), d(L).then((P) => {
        ho = false, Ki = true, Xi = P, v(), p && URL.revokeObjectURL(p);
      }, (P) => {
        ho = false, qp = true, S(P);
      });
...
var mo = O(() => {
  ...
  Ke();
  Tt = (i, e) => {
    let o = xt(), t = o.lengthBytesUTF8(i) + 1, r = o._malloc(t);
...
var Xp = O(() => {
  ...
  jp = (i) => {
    ...
    try {
      ...
      let n = 0;
      return i?.tag !== void 0 && (n = Tt(i.tag, t)), o = e._OrtCreateRunOptions(r.logSeverityLevel, r.logVerbosityLevel, !!r.terminate, n), o === 0 && ht("Can't create run options."), i?.extra !== void 0 && Wr(i.extra, "", /* @__PURE__ */ new WeakSet(), (s, a) => {
        let u = Tt(s, t), l = Tt(a, t);
...
var Jp = O(() => {
  ...
  $g = (i) => {
      ...
      switch (r) {
        case "webnn":
          if (r = "WEBNN", typeof t != "string") {
            let a = t?.deviceType;
            if (a) {
              let u = Tt("deviceType", o), l = Tt(a, o);
        ...
        case "webgpu":
          if (r = "JS", typeof t != "string") {
            ...
            if (s?.preferredLayout) {
              ...
              let a = Tt("preferredLayout", o), u = Tt(s.preferredLayout, o);
    ...
    Bg(r);
    try {
      let n = $g(r.graphOptimizationLevel ?? "all"), s = kg(r.executionMode ?? "sequential"), a = typeof r.logId == "string" ? Tt(r.logId, t) : 0, u = r.logSeverityLevel ?? 2;
      ...
      if (o = e._OrtCreateSessionOptions(n, !!r.enableCpuMemArena, !!r.enableMemPattern, s, !!r.enableProfiling, 0, a, u, l, f), o === 0 && ht("Can't create session options."), r.executionProviders && Fg(o, r.executionProviders, t), r.enableGraphCapture !== void 0) {
        ...
        let p = Tt("enableGraphCapture", t), d = Tt(r.enableGraphCapture.toString(), t);
...
var Ji = O(() => {
  ...
  Hr = (i) => {
    ...
    let o = [-1, 4, 1, 1, 2, 2, 4, 8, -1, 1, 2, 8, 4, 8, -1, -1, -1, -1, -1, -1, -1, 0.5, 0.5][i], t = typeof e == "number" ? e : e.reduce((r, n) => r * n, 1);
  ...
  jr = async (i) => {
    if (typeof i == "string") if (false) try {
      ...
      if (e.code === "ERR_FS_FILE_TOO_LARGE") {
        let { createReadStream: o } = Co("node:fs"), t = o(i), r = [];
...
var qi = O(() => {
  ...
  Cg = (i, e) => {
    ...
    let e = xt(), o = e._malloc(i.byteLength);
    ...
    try {
      ...
      for (let P of e?.executionProviders ?? []) if ((typeof P == "string" ? P : P.name) === "webnn") {
        ...
        if (typeof P != "string") {
          let M = P, V = M?.context, lt = M?.gpuDevice, wt = M?.deviceType, et = M?.numThreads, Dt = M?.powerPreference;
          V ? r.currentContext = V : lt ? r.currentContext = await navigator.ml.createContext(lt) : r.currentContext = await navigator.ml.createContext({ deviceType: wt, numThreads: et, powerPreference: Dt });
        } else r.currentContext = await navigator.ml.createContext();
        break;
      }
      n = await r._OrtCreateSession(o, t, s), n === 0 && ht("Can't create a session."), r.currentContext && (r.jsepRegisterMLContext(n, r.currentContext), r.currentContext = void 0, r.shouldTransferToMLTensor = true);
      let [p, d] = Ng(n), y = !!e?.enableGraphCapture, T = [], v = [], S = [];
      for (let P = 0; P < p; P++) {
      ...
      let L = null;
      return yr.set(n, [n, l, f, L, y, false]), [n, T, v];
    } catch (p) {
  ...
  }, ed = (i, e, o, t, r, n = false) => {
    if (!i) {
      e.push(0);
      return;
    }
    let s = xt(), a = i[0], u = i[1], l = i[3], f, p;
    ...
    let u = a[0], l = a[1], f = a[2], p = a[3], d = a[4], y = a[5], T = e.length, v = t.length, S = 0, L = [], P = [], A = [], M = [], V = s.stackSave(), lt = s.stackAlloc(T * 4), wt = s.stackAlloc(T * 4), et = s.stackAlloc(v * 4), Dt = s.stackAlloc(v * 4);
    try {
      s.jsepOnRunStart?.(u), [S, L] = jp(n);
      for (let Z = 0; Z < T; Z++) ed(o[Z], P, M, i, e[Z], d);
      for (let Z = 0; Z < v; Z++) ed(r[Z], A, M, i, T + t[Z], d);
      let _t = lt / 4, C = wt / 4, Kr = et / 4, we = Dt / 4;
      ...
      for (let Z = 0; Z < v; Z++) {
        ...
        let ie = s.stackSave(), ae = s.stackAlloc(4 * 4), se = false, it, Lt = 0;
        try {
          s._OrtGetTensorData(ge, ae, ae + 4, ae + 8, ae + 12) !== 0 && ht(`Can't access output tensor data on index ${Z}.`);
          let Ye = ae / 4, ue = s.HEAPU32[Ye++];
          Lt = s.HEAPU32[Ye++];
          let Ze = s.HEAPU32[Ye++], De = s.HEAPU32[Ye++], le = [];
          ...
          if (it === "string") {
            ...
            for (let Jt = 0; Jt < Mt; Jt++) {
              let wr = s.HEAPU32[Ot++], Yr = Jt === Mt - 1 ? void 0 : s.HEAPU32[Ot] - wr;
            ...
            let Ot = gt(Lt), Jt = qr(ue, Mt);
            if (Jt === void 0 || !bo(it)) throw new Error(`Unsupported data type: ${it}`);
            se = true, be.push([it, le, { gpuBuffer: Ot, download: s.jsepCreateDownloader(Ot, Jt, it), dispose: () => {
            ...
            let Jt = await gt(Lt, ue, le, false);
            se = true, be.push([it, le, { mlTensor: Jt, download: s.jsepCreateMLTensorDownloader(Lt, it), dispose: () => {
              s.jsepReleaseTensorId(Lt), s._OrtReleaseTensor(ge);
            } }, "ml-tensor"]);
          } else {
            let gt = Zp(it), Ot = new gt(Mt);
      ...
      return p && !d && (s._OrtClearBoundOutputs(p.handle), yr.set(i, [u, l, f, p, d, false])), be;
    } finally {
...
var ta = O(() => {
  ...
  Ke();
  Vr();
  Je = () => !!z.wasm.proxy && typeof document < "u", Xr = false, xo = false, To = false, Qi = /* @__PURE__ */ new Map(), xr = (i, e) => {
    let o = Qi.get(i);
    ...
    switch (i.data.type) {
      case "init-wasm":
        Xr = false, i.data.err ? (To = true, Zi[1](i.data.err)) : (xo = true, Zi[0]()), yo && (URL.revokeObjectURL(yo), yo = void 0);
        break;
      case "init-ep":
      case "copy-from":
      case "create":
    ...
    if (!xo) {
      ...
      if (Xr = true, Je()) return new Promise((i, e) => {
      ...
      try {
        await io(z.wasm), await ao(z), xo = true;
      } catch (i) {
        throw To = true, i;
      } finally {
        Xr = false;
  ...
  ld = (i, e) => {
    ...
    switch (i[3]) {
      ...
      case "gpu-buffer": {
        ...
        return yt.fromGpuBuffer(o, { dataType: e, dims: i[1], download: t, dispose: r });
      }
      case "ml-tensor": {
        let e = i[0];
        ...
        return yt.fromMLTensor(o, { dataType: e, dims: i[1], download: t, dispose: r });
      }
      default:
    ...
    async loadModel(e, o) {
      Fe();
      let t;
      typeof e == "string" ? false ? t = await jr(e) : t = await this.fetchModelAndCopyToWasmMemory(e) : t = e, [this.sessionId, this.inputNames, this.outputNames] = await id(t, o), Ce();
    }
    async dispose() {
    ...
    async run(e, o, t) {
      ...
      Object.entries(e).forEach((d) => {
        let y = d[0], T = d[1], v = this.inputNames.indexOf(y);
        if (v === -1) throw new Error(`invalid input '${y}'`);
        r.push(T), n.push(v);
      });
      let s = [], a = [];
      Object.entries(o).forEach((d) => {
        let y = d[0], T = d[1], v = this.outputNames.indexOf(y);
        if (v === -1) throw new Error(`invalid output '${y}'`);
        s.push(T), a.push(v);
      });
      let u = r.map((d, y) => ld(d, () => `input "${this.inputNames[n[y]]}"`)), l = s.map((d, y) => d ? ld(d, () => `output "${this.outputNames[a[y]]}"`) : null), f = await sd(this.sessionId, n, u, a, l, t), p = {};
...
var pd = {};
Or(pd, { OnnxruntimeWebAssemblyBackend: () => vo, initializeFlags: () => cd, wasmBackend: () => Ug });
var cd;
var vo;
var Ug;
var dd = O(() => {
...
Object.defineProperty(z.versions, "web", { value: Ds, enumerable: true });

// src/worker-asr.ts
var asrSession = null;
var puncSession = null;
var asrCmvn = null;
var tokens = [];
var puncTokens = null;
function post(msg) {
...
function clipFeat(feat, len, dim) {
  const n = len * dim;
  for (let i = 0; i < n; i++) {
    if (!isFinite(feat[i])) feat[i] = 0;
...
async function init(config) {
  ...
  z.wasm.wasmPaths = "https://voice-solo.local/";
  z.wasm.numThreads = 1;
  const wasmBinaries = {
    "ort-wasm-simd-threaded.wasm": config.simdWasm
  };
  const origFetch = self.fetch.bind(self);
  self.fetch = (input, init2) => {
    const url = typeof input === "string" ? input : input.url;
    for (const [name2, buf] of Object.entries(wasmBinaries)) {
      if (url.includes(name2)) {
  ...
  asrSession = await Yd.create(config.asrModelBuffer, {
  ...
  puncSession = await Yd.create(config.puncModelBuffer, {
    executionProviders: ["wasm"],
  ...
  try {
    const warmupAudio = new Float32Array(8e3);
    ...
    if (featLen > 0) {
      ...
      await asrSession.run({
        [asrSession.inputNames[0]]: new yt("float32", feat.slice(0, featLen * dim), [1, featLen, dim]),
        [asrSession.inputNames[1]]: new yt("int32", new Int32Array([featLen]), [1])
      });
    }
  } catch {
  }
...
async function processSegment(audio, startMs, endMs) {
  ...
  try {
    ...
    for (let t = 0; t < outputLen; t++) {
      ...
      for (let v = 0; v < vocabSize; v++) {
        if (logitsData[off + v] > maxV) {
          maxV = logitsData[off + v];
          maxI = v;
        }
      }
      yseq.push(maxI);
    ...
    const filtered = [];
    for (const id2 of yseq) {
      if (id2 !== 0 && id2 !== 2) {
        filtered.push(id2);
        if (filtered.length >= validLen) break;
      }
    }
    let text = filtered.map((id2) => id2 < tokens.length ? tokens[id2] : "").join("");
    text = cleanBpe(text);
    ...
    const perf = {
      ...
      heapMB: performance.memory?.usedJSHeapSize ? Number((performance.memory.usedJSHeapSize / 1024 / 1024).toFixed(0)) : 0
    };
...
async function runPunc(text) {
  ...
  const feeds = {
    [puncSession.inputNames[0]]: new yt("int32", new Int32Array(tokenIds), [1, tokenIds.length]),
    [puncSession.inputNames[1]]: new yt("int32", new Int32Array([tokenIds.length]), [1])
  };
  ...
  for (let t = 0; t < outLen; t++) {
    ...
    for (let c = 0; c < numClasses; c++) {
      if (logitsData[off + c] > maxV) {
        maxV = logitsData[off + c];
        maxI = c;
      }
    }
...
self.onmessage = async (e) => {
  ...
  try {
    ...
    post({ type: "error", message: `ASR: ${e2.message || String(e2)}` });
  }
};
/*!
