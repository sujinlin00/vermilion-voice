...
Local speech recognition plugin for Obsidian — real-time voice transcription using ONNX runtime. Inference runs entirely on-device; first-time model download (~500MB from ModelScope) requires internet. Offline after setup.
...
3. Click "Start" — models will download on first use (~500MB total: VAD 0.5MB + ASR 228MB + PUNC 270MB, with progress display)
...
cd vermilion-voice
npm install
npm run build     # production build → plugin/ + release zip
npm run dev       # dev build (sourcemaps)
```
...
```
vermilion-voice/
├── assets/               Obsidian plugin static assets (manifest, models, styles)
├── src/                  TypeScript source code
├── lib/                  Vendored third-party dependencies (ONNX Runtime, FLAC Encoder)
├── plugin/               Build output (gitignored)
